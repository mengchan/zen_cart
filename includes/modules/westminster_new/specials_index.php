...
if (TEMPLATE_DEBUG == 'True') echo '\includes\modules\TEMPLATE\specials_index.php<br />';//SLT
// initialize vars
...
if(!empty($specials_index_query)){
	$index_title = '<h2 class="centerBoxHeading">' . sprintf(TABLE_HEADING_SPECIALS_INDEX, strftime('%B')) . '</h2>';
	
	if(INDEX_SPECIAL_USE_PRODUCT_LISTING == '2') {
		if (TEMPLATE_DEBUG == 'True') echo 'INDEX_SPECIAL_USE_PRODUCT_LISTING == 2<br /><br />';//SLT
		extract(index_listing($specials_index_query, MAX_DISPLAY_SPECIAL_PRODUCTS_INDEX, SHOW_PRODUCT_INFO_COLUMNS_SPECIALS_PRODUCTS, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, '', TABLE_HEADING_SPECIALS_INDEX, 'zc_show_specials'));
		}
	elseif(INDEX_SPECIAL_USE_PRODUCT_LISTING == '3'){
		if (TEMPLATE_DEBUG == 'True') echo 'INDEX_SPECIAL_USE_PRODUCT_LISTING == 3<br /><br />';//SLT
		$listing_sql = $specials_index_query;
		...
		*/
		...
		$productListingId = 'specialProducts';//this variable not used anywhere
		$is_slt_listing = true;
		$max_index_listing = MAX_DISPLAY_SPECIAL_PRODUCTS_INDEX;
		require($template->get_template_dir('tpl_modules_product_listing.php',DIR_WS_TEMPLATE, $current_page_base,'templates'). '/tpl_modules_product_listing.php');
		$is_slt_listing = false;
	}
	elseif(INDEX_SPECIAL_USE_PRODUCT_LISTING == '1'){
	if (TEMPLATE_DEBUG == 'True') echo 'INDEX_SPECIAL_USE_PRODUCT_LISTING == 1<br /><br />';//SLT
